...
[a, v, Sav, L, MSWD, BIC] = LevenbergMarquardt_LinearRegression_v2...
            (init, data, covmats, assump, maxiter, chiTolerance, lambda0, verbose);

a2s = [0; 2*sqrt(Sav(      1:2*(d-1)+1:end/2))']; % first half of diagonal of Sav
v2s = [0; 2*sqrt(Sav(end/2+d:2*(d-1)+1:end)  )']; % second half of diagonal of Sav

% plot all variables against first, on common x-axis
% black points and blue ellipses are two-sigma confidence intervals about each datum
% black line is the best fit line, red envelope is its two-sigma uncertainty envelope
% data not included in regression are plotted with gray ellipses and points

% For Yuanyuan: don't plot!
% plotlinregNDxl_v3(dataunct, a, v, Sav, skipv, abspct, MSWD)
